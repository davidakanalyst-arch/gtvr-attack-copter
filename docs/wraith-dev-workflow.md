...
- adds the generated cockpit kit: raised textured upholstered front seats, slightly raised and rearward-adjusted compact EC135-style cyclics with rounded heads, detailed collectives with unchanged pivots and slightly raised grips, forward rounded pedals, a forward dashboard without lower shelf/brace tubes, and left/middle/right glass-style panels;
...
The dev wrapper adds a generated cockpit kit directly into the compiled dev `.tmb`. It includes two raised textured upholstered front seats, slightly raised and rearward-adjusted compact dog-leg EC135-style cyclics with rounded control heads, detailed collective/throttle levers with unchanged pivots and grips raised by 0.035 m, forward rounded pedal bars, a forward framed dashboard, and left/middle/right glass-style panels. The current side panels are accepted runtime display receivers: left and right screens populate `DisplayPFDL`/`DisplayPFDR` with independent inset PFD-only atlas windows that place live speed, altitude, attitude and heading-tape data inside the Wraith side screen bezels while excluding the duplicate side strips and allowing lower ND/compass remnants to overflow downward. The centre panel populates `DisplayNDL` from a dedicated `gtvr_center_map_light` texture. Attempts to package an AN2/DR400-style `texture_animation_map_display` sidecar beside the EC135 `.tmq` were ignored by Aerofly. A copied C172 `classic_panel.tmb` panel-option attempt caused a stop cube through duplicate avionics objects and a failed zoom-input link, so the current map attempt packages only a minimal hidden `gtvr_map_panel` option with no copied panel `.tmb` and no extra zoom dynamic object. The dev package also forces the PFD/ND display state inputs on by default and reduces inherited visible stick/collective/pedal click handles in `controls.tmd`, leaving the generated Wraith controls as the visible cockpit controls. Cyclic meshes occupy the EC135 TMQ's proven native `StickL`/`StickR` slots, driven by its existing cyclic-only `StickTransform`/`StickTransformCopilot` controls; collectives remain separate dev graphics bound only to collective travel.

There is intentionally no dash hood/shelf over the panel. The current dashboard/panel group uses `--dash-forward-x-delta 0.55` to hold the dash toward the front of the cockpit without touching the pilot, seats, cyclics, collectives, pedals, or shell/pilot alignment. The seat, collective and pedal cluster uses `--interior-forward-x-delta 0.32` so those parts sit closer to the side-window/pilot area while the dash stays anchored forward. Cyclic floor pivots remain fixed in aircraft coordinates; their swept shafts curve forward around the seat cushions.
