...
- adds the generated cockpit kit: textured upholstered front seats, individual black cyclics, simple black collectives, pedals, a forward dashboard, and left/middle/right glass-style panels;
...
The dev wrapper adds a generated cockpit kit directly into the compiled dev `.tmb`. It includes two textured upholstered front seats, individual black cyclics, simple black collective/throttle levers, pedal pads, a forward framed dashboard, and left/middle/right glass-style panels. The current panel textures are intentionally simple: side flight displays for speed/altitude/attitude and a center rolling-map display. The dev source also populates the EC135 display geometry names `DisplayPFDL`, `DisplayPFDR`, and `DisplayNDL` so the preserved EC135 `.tmq` has named display surfaces to drive if that path is available.
