...
- adds the generated cockpit kit: raised textured upholstered front seats, floor-mounted matte-black cyclics, left-side collectives, forward rounded pedals, a forward dashboard, and left/middle/right glass-style panels;
...
The dev wrapper adds a generated cockpit kit directly into the compiled dev `.tmb`. It includes two raised textured upholstered front seats, floor-mounted matte-black cyclics, left-side collective/throttle levers, forward rounded pedal bars, a forward framed dashboard, and left/middle/right glass-style panels. The current panel textures are intentionally simple: side flight displays for speed/altitude/attitude and a center rolling-map display. The dev source also populates the EC135 display geometry names `DisplayPFDL`, `DisplayPFDR`, and `DisplayNDL` so the preserved EC135 `.tmq` has named display surfaces to drive if that path is available. The dev package also reduces inherited EC135 visible stick/collective/pedal click handles in `controls.tmd`, leaving the generated Wraith controls as the visible cockpit controls. Cyclics are fixed/floor-mounted until a verified cyclic-only visual input path is available; collectives and pedals remain animated.
